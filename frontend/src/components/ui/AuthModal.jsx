...
const FACEBOOK_APP_ID = '2295918557814777'

export default function AuthModal() {
  const { register, login, continueAsGuest, googleLogin, facebookLogin } = useUser()
    ...
    window.FB.login((response) => {
      if (response.authResponse) {
        ...
        facebookLogin(response.authResponse.accessToken)
          .then(() => toast.success('Signed in with Facebook!'))
          .catch((err) => toast.error(err.message || 'Facebook sign-in failed'))
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            <button
              ...
              disabled={loading}
              className="flex items-center justify-center gap-2 py-2.5 rounded-xl border border-border text-xs font-medium text-text-secondary hover:bg-surface-2 hover:border-border/80 transition-colors disabled:opacity-50"
            >
